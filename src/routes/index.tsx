...
function renderMarkdownInline(text: string, keyPrefix: string) {
    ...
    while (rest.length > 0) {
        ...
        if (m === linkM) {
            ...
            nodes.push(
                <a key={`${keyPrefix}-a-${k++}`} href={url} target="_blank"
                   rel="noopener noreferrer"
                   className="underline text-[#9ecfff] hover:text-white">{label}</a>
            )
        ...
        if (m === codeM) {
            const [all, code] = m
            nodes.push(<code key={`${keyPrefix}-c-${k++}`}
                             className="bg-black/40 px-1 rounded font-mono">{code}</code>)
            rest = rest.slice((m.index ?? 0) + all.length)
            continue
        }
        if (m === strongM) {
...
export function renderContent(text: string, openMedia: (g: MediaGallery) => void, openProfile?: (bech: string) => void, openHashtag?: (tag: string) => void, allowedTags?: string[], isAuthorFollowed?: boolean, onOpenNote?: (id: string) => void, onReply?: (e: NDKEvent) => void, onRepost?: (e: NDKEvent) => void, onQuote?: (e: NDKEvent) => void, onOpenThread?: (e: NDKEvent) => void, scopeId?: string, actionMessages?: Record<string, string>, replyOpen?: Record<string, boolean>, replyBuffers?: Record<string, string>, onChangeReplyText?: (id: string, v: string) => void, onCloseReply?: (id: string) => void, onSendReply?: (targetId: string) => void, userFollows?: string[], quoteOpen?: Record<string, boolean>, quoteBuffers?: Record<string, string>, onChangeQuoteText?: (id: string, v: string) => void, onCloseQuote?: (id: string) => void, onSendQuote?: (targetId: string) => void) {
    ...
    return parts.flatMap((part, idx) => {
        ...
        if (subparts.length === 1) return <span
            key={idx}>{renderMarkdownInline(part, `${idx}`)}</span>
        const nodes: any[] = []
        for (let i = 0; i < subparts.length; i++) {
            ...
            if (m) {
                ...
                if (/^(npub1|nprofile1)/i.test(bech) && openProfile) {
                    nodes.push(
                        <InlineProfile key={`${idx}-prof-${i}`} bech={bech}
                                       onOpen={(b) => openProfile(b)}/>
                    )
            ...
            if (openHashtag) {
                ...
                for (let m2: RegExpExecArray | null = re.exec(textSeg); m2; m2 = re.exec(textSeg)) {
                    ...
                    if (before) parts2.push(<span
                        key={`${idx}-t-${i}-${last}`}>{renderMarkdownInline(before, `${idx}-${i}-${last}`)}</span>)
                    const h = m2[0]
                    ...
                    if (shouldLink) {
                        ...
                        parts2.push(<span
                            key={`${idx}-htext-${i}-${start}`}>{h}</span>)
                    }
                    last = end
                }
                const rest2 = textSeg.slice(last)
                if (rest2) parts2.push(<span
                    key={`${idx}-t-${i}-rest`}>{renderMarkdownInline(rest2, `${idx}-${i}-rest`)}</span>)
                nodes.push(<span key={`${idx}-twrap-${i}`}>{parts2}</span>)
            } else {
                nodes.push(<span
                    key={`${idx}-t-${i}`}>{renderMarkdownInline(seg, `${idx}-${i}`)}</span>)
            }
        }
        return nodes
    })
        ...
        <div className="rounded bg-[#10181b]">
            <div className="p-3">
                {!decoded ? (
                    ...
                    <div className="text-sm opacity-70">Loading referenced
                        note…</div>
                ) : !evQuery.data ? (
                    <div className="text-sm opacity-70">Referenced note not
                        found.</div>
                ) : (
                                ...
                                <div className="flex items-center gap-2 justify-end">
                                    {onOpenThread && (
                                        <button type="button"
                                                ...
                                                className="bg-black/60 text-white hover:bg-black/80 text-xs px-2 py-1 rounded-full flex items-center gap-2"
                                                title="Open thread">
                                            <ThreadReelIcon
                                                className="w-8 h-8"/>
                                        </button>
    ...
    return (
        <article className="p-3">
            <div className="flex gap-3">
                <div className="flex-1 min-w-0">
                    <header
                        className="mb-1 flex items-center gap-2 text-sm text-[#cccccc]">
                        <AuthorLabel pubkey={ev.pubkey || ''}
                                     onOpen={(pk) => openProfileByPubkey(pk)}/>
                        <span className="opacity-50">·</span>
                        <time
                            className="opacity-70 hover:underline cursor-pointer"
                            onClick={() => onOpenNote(ev)}
                            title="Open note tab">{formatTime(ev.created_at)}</time>
                    </header>
                    <div
                        className="whitespace-pre-wrap break-words text-[#cccccc]">
                        {ev.kind === 6 ? (
                ...
                <div className="grid grid-cols-[1fr_auto] items-end gap-x-2 gap-y-1 mt-1">
                    ...
                    <div className="flex items-center gap-2 justify-end">
                        ...
                        <button type="button" onClick={() => onRepost(ev)}
                                className="bg-[#1b3a40] text-white text-xs px-2 py-1 rounded-full hover:bg-[#215059] flex items-center gap-2"
                                title="Repost">
                            <RepostEllipsisBubbleIcon className="w-8 h-8"/>
                        </button>
                        <button type="button" onClick={() => onReply(ev)}
                                className={`${(ev.id && replyOpen?.[`${scopeId}|${ev.id}`]) ? 'bg-[#fff3b0] text-black' : 'bg-[#1b3a40] text-white hover:bg-[#215059]'} text-xs px-2 py-1 rounded-full flex items-center gap-2`}
                                title="Reply">
                            <ReplyBubbleIcon className="w-8 h-8"/>
                        </button>
                    </div>
...
function Home() {
    ...
    // Truncation controls for the profile header (user note view header)
    const profileHeaderRef = useRef<HTMLDivElement | null>(null)
    const [isProfileHeaderExpanded, setIsProfileHeaderExpanded] = useState(false)
    const [isProfileHeaderOverflowing, setIsProfileHeaderOverflowing] = useState(false)
    useEffect(() => {
        const el = profileHeaderRef.current
        if (!el) return
        const calc = () => {
            try {
                const threshold = Math.max(0, Math.floor(window.innerHeight * 0.15))
                const natural = el.scrollHeight || 0
                setIsProfileHeaderOverflowing(natural > threshold + 2)
            } catch {
            }
        }
        calc()
    ...
    // Compute top-most visible item timestamp on scroll and save scrollY + topMostTs
    const getTopMostTs = (): number | null => {
        try {
            const nodes = Array.from(document.querySelectorAll('article[data-ts]')) as HTMLElement[]
            // Offset under global header (h-12 => 48px) and potential sticky mode header
            const offset = 48 + 0
            let best: { top: number; ts: number } | null = null
            for (const el of nodes) {
                const rect = el.getBoundingClientRect()
                if (rect.bottom < offset) continue
                const tsStr = el.getAttribute('data-ts') || '0'
                const ts = Number(tsStr)
                if (!best || rect.top < best.top) best = { top: rect.top, ts }
            }
    ...
    useEffect(() => {
        const compute = () => {
            try {
                ...
                const sidebarWidth = window.innerWidth >= 1024 ? 128 : 48 // lg:w-32 vs w-12
                const sidebarGap = window.innerWidth >= 1024 ? 128 + 16 : 48 + 16 // sidebar + gap
    ...
    useEffect(() => {
        try {
            localStorage.setItem('openedProfiles', JSON.stringify(openedProfiles))
        } catch {
        }
    }, [openedProfiles])
    ...
    useEffect(() => {
        try {
            localStorage.setItem('openedHashtags', JSON.stringify(openedHashtags))
        } catch {
        }
    }, [openedHashtags])
    ...
    useEffect(() => {
        try {
            localStorage.setItem('threadTriggerNotes', JSON.stringify(threadTriggerNotes))
        } catch {
        }
    }, [threadTriggerNotes])
    ...
    useEffect(() => {
        try {
            localStorage.setItem('replyBuffers', JSON.stringify(replyBuffers))
        } catch {
        }
    }, [replyBuffers])
    ...
    useEffect(() => {
        try {
            localStorage.setItem('quoteBuffers', JSON.stringify(quoteBuffers))
        } catch {
        }
    }, [quoteBuffers])
    ...
    const openThreadFor = (ev: NDKEvent) => {
        ...
        if (openedThreads[0] === root) {
            if (canFitBoth) {
                setIsThreadsHiddenInWideMode(v => !v)
            } else {
                setIsThreadsModalOpen(v => !v)
            }
            // Also update legacy single-thread state
            setThreadRootId(root)
            _setThreadOpenSeed(ev.id || root)
            return
        }
        ...
        // Add to opened threads stack if not already present (move to top if exists)
        setOpenedThreads(prev => (prev.includes(root) ? [root, ...prev.filter(id => id !== root)] : [root, ...prev].slice(0, 5)))

        // Update legacy single-thread state so existing Thread components work
        setThreadRootId(root)
        _setThreadOpenSeed(triggerNoteId)

        // Open appropriate container depending on width
        if (!canFitBoth) {
            setIsThreadsModalOpen(true)
        } else {
    ...
    const profileMeta = useQuery<{
        ...
        queryFn: async () => {
            ...
            try {
                await withTimeout(u.fetchProfile(), 5000, 'profile fetch')
            } catch {
            }
    ...
    useEffect(() => {
        ...
        if (!meta) return
        setOpenedProfiles(prev => prev.map(p => p.pubkey === profilePubkey ? {
            ...p,
            name: p.name || meta.name,
            picture: p.picture || meta.picture,
            about: p.about || meta.about
        } : p))
    ...
    useEffect(() => {
        ...
        try {
            window.dispatchEvent(new CustomEvent('nostr-active-view', {detail: {label}}))
        } catch {
        }
    ...
    const feedQuery = useInfiniteQuery({
        ...
        queryFn: async ({pageParam}) => {
            ...
            const filters: NDKFilter[] = chunk(follows, 20).map(group => {
                const f: NDKFilter = {
                    kinds: FEED_KINDS_NO_REACTIONS,
                    authors: group as any,
                    limit: PAGE_SIZE
                }
    ...
    useEffect(() => {
        try {
            window.dispatchEvent(new CustomEvent('nostr-hashtags-cache', {detail: {tags: cachedHashtags}}))
        } catch {
        }
    }, [cachedHashtags])
    ...
    return (
        <>
            ...
            {canFitSidebar && (
                <div
                    className="fixed left-0 top-12 z-40 flex flex-col gap-2 p-0 bg-black h-full">
                    {/* Opened note tabs */}
                    {openedNotes.map((n) => (
                        <div key={n.id} className="relative inline-block group">
                            <button
                                ...
                                className={`w-40 h-12 ${mode === 'note' && currentNoteId === n.id ? 'bg-[#162a2f]' : 'bg-black hover:bg-[#1b3a40]'} flex items-center justify-start px-3`}
                                title={`Open note ${n.id}`}
                            >
                                <ThreadReelIcon
                                    className="w-6 h-6 text-[#cccccc]"/>
                                <span
                                    className="ml-2 text-[#cccccc] select-none truncate">Note {n.id.slice(0, 8)}…</span>
                            </button>
                            <button
                                type="button"
                    ...
                    {openedProfiles.filter(p => !(user && p.pubkey === user.pubkey)).map((p) => (
                        <div key={p.pubkey}
                            ...
                            <button
                                ...
                                className={`w-12 lg:w-40 h-12 lg:h-12 ${mode === 'profile' && profilePubkey === p.pubkey ? 'bg-[#162a2f]' : 'bg-black hover:bg-[#1b3a40]'} flex items-center justify-center lg:justify-start lg:px-3`}
                                title={`Open ${p.name || p.npub || 'profile'}`}
                            >
                                {p.picture ? (
                                    <img src={p.picture} alt="avatar"
                                ...
                                <span
                                    className="hidden lg:inline ml-2 text-[#cccccc] select-none truncate">{p.name || (p.npub ? p.npub.slice(0, 10) + '…' : shorten(p.pubkey))}</span>
                            </button>
                            <div className="hidden">
                                <span
                                    className="text-[#cccccc] select-none truncate">{p.name || (p.npub ? p.npub.slice(0, 10) + '…' : shorten(p.pubkey))}</span>
                            </div>
                    ...
                    {openedHashtags.map((t) => (
                        <div key={`tag-${t}`}
                            ...
                            <button
                                ...
                                className={`w-12 lg:w-40 h-12 ${mode === 'hashtag' && currentHashtag === t ? 'bg-[#162a2f]' : 'bg-black hover:bg-[#1b3a40]'} flex items-center justify-start px-3 relative overflow-hidden`}
                                title={`Open #${t}`}
                            >
                                <span
                                    className="text-[#cccccc] select-none whitespace-nowrap">#{t}</span>
                                <div
                                    className={`absolute right-0 top-0 w-3 h-full bg-gradient-to-l ${mode === 'hashtag' && currentHashtag === t ? 'from-[#162a2f]' : 'from-black group-hover:from-[#1b3a40]'} to-transparent pointer-events-none`}></div>
                            </button>
                            <button
                                type="button"
                    ...
                    {user && (
                        <div className="relative inline-block group">
                            <button
                                ...
                                className={`w-12 lg:w-40 h-12 ${mode === 'follows' ? 'bg-[#162a2f]' : 'bg-black hover:bg-[#1b3a40]'} flex items-center justify-center lg:justify-start lg:px-3`}
                                title={'Show posts from people you follow'}
                            >
                                <UsersIcon className="w-6 h-6 text-[#cccccc]"/>
                                <span
                                    className="hidden lg:inline ml-2 text-[#cccccc] select-none">Follows</span>
                            </button>
                        </div>
                    )}
                    {user && (
                    ...
                    <div className="relative inline-block group">
                        <button
                            ...
                            className={`w-12 lg:w-40 h-12 ${mode === 'global' ? 'bg-[#162a2f]' : 'bg-black hover:bg-[#1b3a40]'} flex items-center justify-center lg:justify-start lg:px-3`}
                        >
                        ...
                        <div
                            ...
                            {mode === 'profile' && (
                                <div
                                    ...
                                    className={`sticky top-12 z-40 w-full flex ${profileMeta.data?.banner ? '' : 'bg-[#1a2529]'} relative ${(!isProfileHeaderExpanded && isProfileHeaderOverflowing) ? 'overflow-hidden' : ''}`}
                                    style={!isProfileHeaderExpanded && isProfileHeaderOverflowing ? {maxHeight: '15vh'} as any : undefined}
                                >
                                    {/* Banner background */}
                                    {profileMeta.data?.banner && (
                                        <div
                                    ...
                                    {profileMeta.data?.banner && <div
                                        className="absolute inset-0 bg-black/30"
                                        aria-hidden="true"/>}
                                    <div
                                        className="relative p-4 flex items-start gap-4 w-full">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div
                                                className="bg-black/60 rounded p-[0.5em]">
                                                ...
                                                {profileMeta.data?.about ? (
                                                    <div
                                                        className="whitespace-pre-wrap break-words text-[#f0f0f0] text-[0.6667em]">
                                                        {profileMeta.data.about}
                                                    </div>
                                                ) : null}
                                            </div>
                                    ...
                                    {/* Bottom gradient when truncated */}
                                    {(!isProfileHeaderExpanded && isProfileHeaderOverflowing) && (
                                        <div
                                            className="pointer-events-none absolute left-0 right-0 bottom-0 h-16 bg-gradient-to-t from-[#1a2529] to-transparent"/>
                                    )}
                                    ...
                                    {(!isProfileHeaderExpanded && isProfileHeaderOverflowing) && (
                                        <div
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setIsProfileHeaderExpanded(true)}
                                                className="text-xs px-2 py-1 rounded bg-black/60 hover:bg-black/80 text-[#cccccc]"
                                                title="Show more"
                                            >
                                                Show more
                                            </button>
                            ...
                            {mode === 'note' && (
                                <div
                                    ...
                                    <div
                                        ...
                                        <div
                                            className="flex items-center gap-3 text-xl text-[#f0f0f0]">
                                            <ThreadReelIcon
                                                className="w-6 h-6"/>
                                            <div
                                                className="font-semibold">Note
                                            </div>
                                        </div>
                                        <div className="ml-2">
                                            <button
                                    ...
                                    >
                                        {(pullDistance > 0 || isFetchingNewer) && (
                                            <div
                                                ...
                                                <Spinner/>
                                                <span
                                                    className="text-sm">{isFetchingNewer ? 'Refreshing…' : pullDistance >= PULL_THRESHOLD ? 'Release to refresh' : 'Pull to refresh'}</span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    >
                                        {(bottomPullDistance > 0 || feedQuery.isFetchingNextPage) && (
                                            <div
                                                ...
                                                <Spinner/>
                                                <span
                                                    className="text-sm">{feedQuery.isFetchingNextPage ? 'Loading more…' : bottomPullDistance >= BOTTOM_PULL_THRESHOLD ? 'Release to load more' : 'Pull up to load more'}</span>
                                            </div>
                                        )}
                                    </div>
                ...
                {openedThreads.length > 0 && canFitBoth && !isThreadsHiddenInWideMode && (
                    <div
                        ...
                        <div
                            ...
                            <div
                                className="px-4 py-3 bg-[#1a2529] border-b border-[#37474f]">
                                <div
                                    className="flex items-center justify-between">
                                    <div
                                        className="flex items-center gap-2 text-[#fff3b0] font-semibold">
            ...
            {openedThreads.length > 0 && !canFitBoth && isThreadsModalOpen && (
                <div
                    ...
                    {/* Modal content */}
                    <div
                        className="relative w-full max-w-2xl h-full bg-[#162a2f] shadow-2xl flex flex-col mr-4 mt-4 mb-4 rounded-xl overflow-hidden">
                        {/* Header with close button */}
                        <div
                            className="px-4 py-3 bg-[#1a2529] border-b border-[#37474f] flex items-center justify-between">
                            <div
                                className="flex items-center gap-2 text-[#fff3b0] font-semibold">
                                <ThreadReelIcon className="w-5 h-5"/>
                                Opened Threads
            ...
            {!canFitSidebar && isSidebarDrawerOpen && (
                <div
                    className="fixed inset-0 z-50 flex items-start justify-start bg-black/60">
                    {/* Modal backdrop - click to close */}
                    <div
                        className="absolute inset-0"
                    ...
                    {/* Modal content */}
                    <div
                        className="relative w-full max-w-2xl h-full bg-[#162a2f] shadow-2xl flex flex-col ml-4 mt-4 mb-4 rounded-xl overflow-hidden">
                        {/* Header with close button */}
                        <div
                            className="px-4 py-3 bg-[#1a2529] border-b border-[#37474f] flex items-center justify-between">
                            <div
                            ...
                            <div className="space-y-2">
                                ...
                                {openedProfiles.filter(p => !(user && p.pubkey === user.pubkey)).map((p) => (
                                    <div key={p.pubkey}
                                        ...
                                        <button
                                            ...
                                            className={`w-full h-12 ${mode === 'profile' && profilePubkey === p.pubkey ? 'bg-[#162a2f]' : 'bg-black hover:bg-[#1b3a40]'} flex items-center justify-start px-3 rounded`}
                                            title={`Open ${p.name || p.npub || 'profile'}`}
                                        >
                                            {p.picture ? (
                                                <img src={p.picture}
                                            ...
                                            <span
                                                className="ml-2 text-[#cccccc] select-none truncate">{p.name || (p.npub ? p.npub.slice(0, 10) + '…' : shorten(p.pubkey))}</span>
                                        </button>
                                        <button
                                            type="button"
                                            aria-label="Close profile tab"
                                ...
                                {openedHashtags.map((t) => (
                                    <div key={`tag-${t}`}
                                        ...
                                        >
                                            <span
                                                className="text-[#cccccc] select-none truncate">#{t}</span>
                                        </button>
                                        <button
                                            type="button"
                                            aria-label="Close hashtag tab"
                                ...
                                {user && (
                                    ...
                                    >
                                        <UsersIcon
                                            className="w-6 h-6 text-[#cccccc]"/>
                                        <span
                                            className="ml-2 text-[#cccccc] select-none">Follows</span>
                                    </button>
                                )}
                                {user && (
                                    <button
                                    ...
                                    >
                                        <BellIcon
                                            className="w-6 h-6 text-[#cccccc]"/>
                                        <span
                                            className="ml-2 text-[#cccccc] select-none">Notifications</span>
                                    </button>
                                )}
                                <button
                                    aria-label="Global feed"
                                ...
                                >
                                    <GlobeIcon
                                        className="w-6 h-6 text-[#cccccc]"/>
                                    <span
                                        className="ml-2 text-[#cccccc] select-none">Global</span>
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            {/* Right-side thread panel (fixed) for wide and overlay for narrow) */}
            {openedThreads.length > 0 && ((canFitBoth && !isThreadsHiddenInWideMode) || (!canFitBoth && isThreadsModalOpen)) && (
                <div
                    className="fixed top-0 right-0 h-screen w-full sm:w-[min(100vw,512px)] max-w-2xl z-[1500] p-2">
                    <div
                        className="h-full bg-[#0f1a1d] rounded-lg shadow-2xl border border-black/50 flex flex-col">
                        <div
                            className="sticky top-0 z-10 flex items-center justify-between px-3 py-2 border-b border-black/50">
                            <div
                                className="text-[#fff3b0] font-semibold">threads
                            </div>
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    onClick={() => {
                                        if (canFitBoth) {
                                            setIsThreadsHiddenInWideMode(true)
                                        } else {
                                            setIsThreadsModalOpen(false)
                                        }
                                    }}
                                    className="px-3 py-1 rounded bg-black/70 text-white hover:bg-black/90"
                                    title="Hide threads"
                                    aria-label="Hide threads"
                                >
                                    Hide
                        ...
                        <div className="flex-1 min-h-0 overflow-hidden">
                            <div className="h-full overflow-auto p-2">
                                <ThreadModal
                                    rootId={openedThreads[0]}
                                    seedId={threadTriggerNotes[openedThreads[0]]}
                                    onClose={() => {
                                        // Close removes from stack and hides
                                        closeThreadFromStack(openedThreads[0])
                                        if (!canFitBoth) setIsThreadsModalOpen(false)
                                    }}
                                    openMedia={setMediaToShow}
                                    openProfileByBech={openProfileByBech}
                                    openProfileByPubkey={openProfileByPubkey}
                                    ...
                                    onChangeReplyText={changeReplyText}
                                    onCloseReply={closeReply}
                                    onSendReply={(id) => sendReply(`thread-modal:${openedThreads[0]}|${id}`)}
                                    activeReplyTargetId={threadActiveReplyTargetKey ? threadActiveReplyTargetKey.split('|')[1] : null}
                                    replyText={threadActiveReplyTargetKey ? (replyBuffers[threadActiveReplyTargetKey] || '') : ''}
                                    onChangeThreadReplyText={changeThreadReplyText}
                                    onCloseThreadReply={closeThreadReply}
            ...
            {/* Floating restore button (FAB) when threads are hidden/minimized) */}
            {openedThreads.length > 0 && ((canFitBoth && isThreadsHiddenInWideMode) || (!canFitBoth && !isThreadsModalOpen)) && (
                <button
                    type="button"
                    onClick={() => {
                        if (canFitBoth) setIsThreadsHiddenInWideMode(false); else setIsThreadsModalOpen(true)
                    }}
                    className="fixed bottom-4 right-4 z-[1600] w-12 h-12 rounded-full bg-[#1b3a40] text-white hover:bg-[#215059] shadow-lg flex items-center justify-center"
                    title="Show threads"
                    aria-label="Show threads"
                >
                    <ThreadReelIcon className="w-7 h-7"/>
            ...
            {mediaToShow && (
                <MediaModal gallery={mediaToShow}
                            onClose={() => setMediaToShow(null)}/>
            )}
        </>
    )
}
...
function ThreadModal({
    ...
    onCloseReply?: () => void;
    onChangeThreadReplyText?: (v: string) => void;
    ...
    return (
        <div className="fixed inset-0 z-[1200] bg-black/70" onClick={onBackdrop}
                ...
                <div className="p-4 pt-[1em] absolute top-0 left-0 right-0">
                    ...
                    {!root ? (
                        ...
                        <div className="space-y-4">
                                    ...
                                    <div key={id}
                                        ...
                                        <div className="p-3">
                                            <div className="flex gap-3">
                                                <div className="flex-1 min-w-0">
                                                    <header
                                                        className="mb-2 flex items-center gap-2 text-xs text-[#cccccc]">
                                                        <AuthorLabel
                                                            pubkey={ev.pubkey || ''}
                                                            onOpen={(pk) => openProfileByPubkey(pk)}/>
                                                        <span
                                                        ...
                                                        <time
                                                            className="opacity-70">{formatTime(ev.created_at)}</time>
                                                    </header>
                                                    {ev.kind === 6 ? (
                                                        <RepostNote
                                                            ev={ev}
                            ...
                            {truncated && (
                                <div
                                    className="text-xs text-[#cccccc] opacity-70">Thread
                                    truncated due to size. Some replies may be
                                    hidden.</div>
                            )}
                        </div>
                    )}
...
function NoteCard_Legacy({
    ...
    hideThread?: boolean;
    userPubkey?: string;
    showActionMessage?: (e: NDKEvent, msg: string) => void;
    repostMode?: Record<string, boolean>;
    onCancelRepost?: (e: NDKEvent) => void;
    quoteOpen?: Record<string, boolean>;
    quoteBuffers?: Record<string, string>;
            ...
            <div className="flex flex-col">
                <div className="flex gap-3 mb-3">
                    <div className="flex-1 min-w-0">
                        <header
                            ...
                            {mentionsUser && (
                                <>
                                    <span className="opacity-50">·</span>
                                    <span
                                        className="text-[#9ecfff] text-xs px-1.5 py-0.5 bg-[#1b3a40] rounded-full">{ev.kind === 6 ? 'reposted' : 'reply'}</span>
                                </>
                            )}
                            <button
                                type="button"
                        ...
                        {jsonViewerOpen && (
                            <div
                                className="mb-3 bg-black/40 border border-[#37474f] rounded-lg overflow-hidden">
                                <div
                                    className="bg-[#1a2529] px-3 py-2 border-b border-[#37474f] text-xs text-[#cccccc] font-medium">
                                    Event JSON
                                </div>
                                <div className="max-h-96 overflow-auto">
                  <pre
                      className="p-3 text-xs text-[#cccccc] whitespace-pre-wrap break-all font-mono leading-relaxed">
                    {JSON.stringify({
                        id: ev.id,
                        pubkey: ev.pubkey,
                        ...
                        >
                            ...
                            {!expanded && isOverflowing && (
                                <div
                                    className="absolute left-0 right-0 bottom-0 h-16 bg-gradient-to-t from-[#162a2f] to-transparent pointer-events-none"
                                    aria-hidden="true"/>
                            )}
                        </div>
                        ...
                        {!expanded && isOverflowing && (
                            <div
                                className="mt-2 flex justify-center relative z-20">
                                <button
                                    type="button"
                    ...
                    <div
                        className="flex flex-col items-end gap-2 flex-shrink-0 self-start">
                        {showThreadButton ? (
                            <button type="button"
                                    onClick={() => onOpenThread(ev)}
                                    className={`${isActiveThread ? 'bg-[#fff3b0] text-black' : 'bg-black/60 text-white hover:bg-black/80'} text-xs px-2 py-1 rounded-full flex items-center gap-2`}
                                    title="Open thread">
                ...
                <div ref={buttonRowRef}
                     className="flex justify-end items-center w-full gap-2">
                    <button type="button" onClick={() => handleQuote(ev)}
                            className={`${(ev.id && quoteOpen?.[`quote|${ev.id}`]) ? 'bg-[#fff3b0] text-black' : 'bg-[#1b3a40] text-white hover:bg-[#215059]'} text-xs px-2 py-1 rounded-full flex items-center justify-center`}
                            title="Quote">
                        <QuoteIcon className="w-8 h-8"/>
                    </button>
                    ...
                    {repostMode?.[ev.id || ''] ? (
                        <div className="flex items-center gap-1">
                            <button type="button" onClick={() => onRepost(ev)}
                                    className="bg-[#fff3b0] text-black text-xs px-2 py-1 rounded-full hover:bg-[#ffed80] flex items-center justify-center"
                                    title="Repost (active)">
                                <RepostEllipsisBubbleIcon className="w-8 h-8"/>
                            </button>
                            <button type="button"
                                    onClick={() => onCancelRepost?.(ev)}
                                    className="bg-red-600 text-white text-xs px-2 py-1 rounded-full hover:bg-red-700 flex items-center justify-center"
                                    title="Cancel repost">
                                ×
                            </button>
                        </div>
                        ...
                        <button type="button" onClick={() => onRepost(ev)}
                                className="bg-[#1b3a40] text-white text-xs px-2 py-1 rounded-full hover:bg-[#215059] flex items-center justify-center"
                                title="Repost">
                            <RepostEllipsisBubbleIcon className="w-8 h-8"/>
                        </button>
                    )}

                    <button type="button" onClick={() => handleReply(ev)}
                            className={`${(ev.id && replyOpen?.[`${scopeId}|${ev.id}`]) ? 'bg-[#fff3b0] text-black' : 'bg-[#1b3a40] text-white hover:bg-[#215059]'} text-xs px-2 py-1 rounded-full flex items-center justify-center`}
                            title="Reply">
                        <ReplyBubbleIcon className="w-8 h-8"/>
                    </button>
                </div>
...
export function ReactionButtonRow({eventId, onReact, excludeEl}: {
    eventId: string;
    ...
    const handleReactClick = async () => {
        ...
        setModalPosition({
            x: MARGIN,
            y: window.innerHeight - MARGIN - MODAL_HEIGHT
        })
        setIsEmojiModalOpen(true)
    }
    ...
    return (
        <div className="mt-3 flex items-end gap-2 flex-wrap-reverse">
                ...
                <>
                    ...
                    <div
                        ...
                        style={{
                            top: `${holeTop}px`,
                            height: `${holeHeight}px`,
                            width: `${Math.max(0, window.innerWidth - holeRight)}px`
                        }}
                        aria-hidden="true"
                    />
                </>
...
function MediaModal({gallery, onClose}: {
        ...
        <div className="fixed inset-0 z-[2000] bg-black/80 flex flex-col"
            ...
            <div className="flex justify-end gap-2 p-2"
                ...
                <button onClick={(e) => {
                    ...
                    setZoom(z => Math.max(0.25, +(z - 0.25).toFixed(2)))
                        ...
                        className="bg-[#162a2f] text-white px-3 py-1 rounded hover:bg-[#1b3a40]"
                        aria-label="Zoom out">-
                </button>
                <button onClick={(e) => {
                    e.stopPropagation();
            ...
            <div
                ...
                <div className="max-w-[95vw] max-h-[85vh] overflow-auto"
                     onClick={(e) => e.stopPropagation()}>
                    {current.type === 'image' ? (
                        <img src={current.url} alt="media" style={{
                        ...
                        <video src={current.url} controls autoPlay
                               className="max-w-full max-h-[85vh]" style={{
                            transform: `scale(${zoom})`,
                            transformOrigin: 'center center'
                        }}/>
                    )}
                ...
                <div className="flex gap-3">
                    <div className="flex-1 min-w-0">
                        <div
                            className="mb-2 flex items-center gap-2 text-xs text-[#cccccc]">
                            {target?.pubkey ?
                                <AuthorLabel pubkey={String(target.pubkey)}
                                             onOpen={(pk) => openProfileByPubkey(pk)}/> :
                                <span
                                    className="opacity-70">Unknown author</span>}
                            <span className="opacity-50">·</span>
                            <time
                                className="opacity-70 hover:underline cursor-pointer"
                                onClick={() => onOpenNote?.(targetEvent)}
                                title="Open note tab">{formatTime((target as any)?.created_at)}</time>
                        </div>
                        {!target ? (
                            <div
                                className="text-xs text-[#cccccc]">Loading…</div>
                        ) : (
                            <div
                                className="whitespace-pre-wrap break-words text-[#cccccc]">
                        ...
                        {onReply && onRepost && onQuote && (
                            <div
                                ...
                                <div className="flex items-center gap-2 justify-end">
                                    ...
                                    <button type="button"
                                            onClick={() => onReply(targetEvent)}
                                            className={`${(targetEvent.id && scopeId && replyOpen?.[`${scopeId}|${targetEvent.id}`]) ? 'bg-[#fff3b0] text-black' : 'bg-[#1b3a40] text-white hover:bg-[#215059]'} text-xs px-2 py-1 rounded-full flex items-center gap-2`}
                                            title="Reply">
                                        <ReplyBubbleIcon className="w-8 h-8"/>
                                    </button>
                                </div>
...
function InlineProfile({bech, onOpen}: {
    bech: string;
    onOpen: (bech: string) => void
}) {
    const {data} = useQuery<{
        pubkey: string;
        name: string;
        ...
        queryFn: async () => {
            try {
                ...
                try {
                    await withTimeout(u.fetchProfile?.(), 4000, 'inline profile fetch')
                } catch {
                }
    ...
    return (
        ...
        >
            {pic ? <img src={pic} alt="avatar"
                ...
                <UserIcon className="w-8 h-8 p-1 rounded-full bg-black/30"/>}
...
export function AuthorLabel({pubkey, onOpen}: {
    pubkey: string,
                ...
                {pic ? (
                    <span className="inline-flex items-center justify-center rounded-full">
                        <img src={pic} alt="avatar"
                             className="w-[3rem] h-[3rem] p-1 rounded-full object-cover"/>
                    </span>
                ) : null}
                <span className="opacity-90 text-[1.33em] p-2">{name}</span>
            </button>
        )
    }
    return (
...
function ReposterAvatar({pubkey, onOpen}: {
    pubkey: string;
    onOpen?: (pubkey: string) => void
}) {
    const {data} = useQuery({
        ...
        queryFn: async () => {
            try {
                const user = ndk.getUser({pubkey})
                try {
                    await withTimeout(user.fetchProfile(), 4000, 'profile pic fetch')
                } catch {
                }
                const prof: any = user.profile || {}
                const picture: string | undefined = prof.picture || undefined
    ...
    const body = (
        <span
            className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-black/40 overflow-hidden">
      {pic ? <img src={pic} alt="avatar" className="w-6 h-6 object-cover"/> :
          <UserIcon className="w-4 h-4 opacity-60"/>}
    </span>
    )
    if (onOpen) {
        return (
            <button type="button" onClick={() => onOpen(pubkey)}
                    className="focus:outline-none" title={shorten(pubkey, 12)}>
                {body}
            </button>
        )
    }
...
function GlobeIcon({className = ''}: { className?: string }) {
    return (
        <svg className={className} viewBox="0 0 24 24" fill="none"
            ...
            <path
                d="M3 12h18M12 3c2.5 2.5 3.75 5.5 3.75 9S14.5 17.5 12 21M12 3C9.5 5.5 8.25 8.5 8.25 12S9.5 17.5 12 21"
                stroke="currentColor" strokeWidth="1.5"/>
        </svg>
    )
}
...
function UsersIcon({className = ''}: { className?: string }) {
    return (
        <svg className={className} viewBox="0 0 24 24" fill="none"
             xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <path
                d="M9 11c2.209 0 4-1.791 4-4S11.209 3 9 3 5 4.791 5 7s1.791 4 4 4z"
                stroke="currentColor" strokeWidth="1.5"/>
            <path d="M2 21c0-3.314 2.686-6 6-6h2c3.314 0 6 2.686 6 6"
                  stroke="currentColor" strokeWidth="1.5"/>
            <path d="M17 11c1.657 0 3-1.343 3-3s-1.343-3-3-3"
                  stroke="currentColor" strokeWidth="1.5"/>
            <path d="M22 21c0-2.209-1.791-4-4-4" stroke="currentColor"
                  strokeWidth="1.5"/>
        </svg>
    )
}
    ...
    return (
        <svg className={className} viewBox="0 0 24 24" fill="none"
            ...
            <rect x="6" y="5" width="12" height="14" rx="2"
                  stroke="currentColor" strokeWidth="1.5"/>
            <path d="M8 8h8M8 11h8M8 14h8M8 17h6" stroke="currentColor"
...
export function ReplyBubbleIcon({className = ''}: { className?: string }) {
    ...
    return (
        <svg className={className} viewBox="0 0 24 24" fill="none"
             xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <path
                d="M4 5h12a4 4 0 0 1 4 4v2a4 4 0 0 1-4 4H10l-5 4v-4H4a4 4 0 0 1-4-4V9a4 4 0 0 1 4-4z"
                transform="translate(2 1)" stroke="currentColor"
...
export function QuoteIcon({className = ''}: { className?: string }) {
    ...
    return (
        <svg className={className} viewBox="0 0 24 24" fill="none"
             xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <path d="M9 7c-2.761 0-5 2.239-5 5v5h5v-5H7c0-1.105.895-2 2-2V7z"
                  fill="currentColor"/>
            <path d="M20 7c-2.761 0-5 2.239-5 5v5h5v-5h-2c0-1.105.895-2 2-2V7z"
                  fill="currentColor"/>
        </svg>
    )
}

export function RepostEllipsisBubbleIcon({className = ''}: {
    className?: string
}) {
    ...
    return (
        <svg className={className} viewBox="0 0 24 24" fill="none"
             xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <path
                d="M4 5h12a4 4 0 0 1 4 4v3a4 4 0 0 1-4 4H10l-5 4v-4H4a4 4 0 0 1-4-4V9a4 4 0 0 1 4-4z"
                transform="translate(2 1)" stroke="currentColor"
                strokeWidth="1.5" strokeLinejoin="round"/>
...
function EmojiIcon({className = ''}: { className?: string }) {
    return (
        <svg className={className} viewBox="0 0 24 24" fill="none"
            ...
            <path d="M8 14c1.333 1 2.667 1 4 0s2.667-1 4 0"
                  stroke="currentColor" strokeWidth="1.5"
                  strokeLinecap="round"/>
        </svg>
    )
...
function SendIcon({className = ''}: { className?: string }) {
    return (
        <svg className={className} viewBox="0 0 24 24" fill="none"
             xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <path d="M3 11l18-8-8 18-2-7-8-3z" stroke="currentColor"
    ...
    const {data: profile} = useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                try {
                    await withTimeout(user.fetchProfile(), 4000, 'compact reaction profile fetch')
                } catch {
                }
                const prof: any = user.profile || {}
                const name: string = prof.displayName || prof.display_name || prof.name || prof.nip05 || ''
            ...
            >
                ...
                <div className="relative flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0 flex items-center gap-2">
                        ...
                        <span
                            className="text-sm text-[#999] ml-auto">{formatTime(ev.created_at)}</span>
                    </div>
                </div>
            </div>
...
export function ReplyComposer({value, onChange, onClose, onSend, replyKey}: {
    value: string;
...
export function QuoteComposer({value, onChange, onClose, onSend, quoteKey}: {
    value: string;
    onChange: (v: string) => void;
    onClose: () => void;
    onSend: () => void;
    ...
    if (openedThreads.length === 0) {
        return (
            <div className="p-6 text-center text-[#cccccc] opacity-60">
                ...
                <div className="text-sm">Open threads from the main feed to see
                    them here as a stack
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4 max-h-screen overflow-y-auto">
            {openedThreads.map((rootId, index) => (
                <div key={rootId}
                     className="bg-[#0f1a1d] rounded-lg shadow-xl overflow-hidden w-full">
                    <div
                        className="flex items-center justify-between px-3 py-2 border-b border-black/50 bg-[#1a2529]">
                        <button
                            type="button"
              ...
              <span className="text-xs">
                {foldedThreads[rootId] ? '▼' : '▲'}
                        ...
                        <button type="button"
                                onClick={() => onCloseThread(rootId)}
                                className="w-8 h-8 rounded-full bg-black/70 text-white hover:bg-black/90"
                ...
                {order.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        ...
                        {triggerNote && triggerNoteId !== rootId && (
                            <div
                                className="border-2 border-[#fff3b0] rounded-lg bg-[#fff3b0]/10 overflow-hidden">
                                <div
                                    className="px-3 py-1 bg-[#fff3b0]/20 text-[#fff3b0] text-sm font-medium">
                        ...
                        {order.length > 10 && (
                            <div
                                className="text-sm text-[#cccccc] opacity-60 text-center">
...
function ThreadPanel({
    ...
    return (
        <div className="bg-[#0f1a1d] rounded-lg shadow-xl overflow-hidden">
            <div
                className="flex items-center justify-between px-3 py-2 border-b border-black/50">
                <div className="text-[#fff3b0] font-semibold">threads</div>
